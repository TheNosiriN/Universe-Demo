...
vec4 sample_biquadratic(vec2 res, vec2 uv){
    vec2 q = fract(uv * res);
    vec2 c = (q*(q - 1.0) + 0.5) / res;
    vec2 w0 = uv - c;
    vec2 w1 = uv + c;
    vec4 s = texture(normals_tex, vec2(w0.x, w0.y))
               + texture(normals_tex, vec2(w0.x, w1.y))
               + texture(normals_tex, vec2(w1.x, w1.y))
               + texture(normals_tex, vec2(w1.x, w0.y));
    return s / 4.0;
}
...
void main(){
	...
	FragColor.xyz = vec3(1) * max(0.0, dot(normal, lightDir));
	FragColor.a = 1.0;
}
